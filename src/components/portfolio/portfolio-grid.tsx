...
export function PortfolioGrid(): React.ReactElement {
  ...
  return (
    <div className="space-y-10">
      <div className="section-anchor section-anchor-muted rounded-[2rem] p-6">
        <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {filterOptions[key].map((option) => (
                    <button
                      ...
                      className={`rounded-full border border-transparent px-3 py-1 text-xs font-semibold transition-all ${
                        filters[key] === option
                          ? "bg-primary/10 text-primary shadow-[inset_0_0_0_1px_hsla(var(--primary),0.12)]"
                          : "bg-white/62 text-muted-foreground shadow-[inset_0_0_0_1px_hsla(var(--border),0.14)] hover:text-foreground hover:bg-white/80 dark:bg-surface/70 dark:shadow-[inset_0_0_0_1px_hsla(var(--border),0.24)]"
                      }`}
                    >
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            {hasActiveFilters && (
              <button
                type="button"
                className="inline-flex items-center gap-2 rounded-full border border-transparent bg-primary/10 px-4 py-2 text-xs font-semibold text-primary shadow-[inset_0_0_0_1px_hsla(var(--primary),0.12)] transition-shadow hover:shadow-md"
      ...
      <div className="grid gap-4">
        {filteredProjects.map((project) => {
          ...
          return (
            <article
              key={project.slug}
              className="compact-tile grid gap-5 rounded-[1.9rem] px-5 py-5 shadow-soft lg:grid-cols-[128px,1fr] lg:items-start"
            >
              <Link
                href={`/portfolio/${project.slug}`}
                className="group relative aspect-square overflow-hidden rounded-[1.15rem] bg-surface-2"
              >
                <Image
                  src={project.image}
              ...
              <div className="space-y-5">
                ...
                <div className="space-y-2">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    {metricHighlight ? (
                      <div className="rounded-full bg-primary/10 px-3 py-2 text-xs font-semibold text-primary shadow-[inset_0_0_0_1px_hsla(var(--primary),0.1)]">
      ...
      {filteredProjects.length === 0 && (
        <div className="section-anchor section-anchor-muted rounded-[2rem] p-10 text-center">
